            ...
            <div class="row">
                <div class="col-lg-8">
                    <div class="mb-3">
                        ...
                        <input type="text" name="judul" class="form-control" value="<?= old('judul', $berita['judul']) ?>" required placeholder="Ketik judul berita atau kegiatan...">
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-semibold">Konten / Isi Lengkap *</label>
                        <textarea name="konten" id="editor" class="form-control" placeholder="Tulis isi berita atau kegiatan secara lengkap..."><?= old('konten', $berita['konten']) ?></textarea>
                    </div>
                </div>
                    ...
                    <div class="card bg-light border-0 p-3 mb-3">
                        <div class="mb-3">
                            ...
                            <select name="kategori" class="form-select" required>
                                <option value="Berita" <?= old('kategori', $berita['kategori']) === 'Berita' ? 'selected' : '' ?>>Berita / Artikel</option>
                                <option value="Kegiatan" <?= old('kategori', $berita['kategori']) === 'Kegiatan' ? 'selected' : '' ?>>Kegiatan / Acara</option>
                            </select>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <?php if ($berita['gambar']) : ?>
                                <div class="mb-2">
                                    <img src="<?= base_url('uploads/berita/' . esc($berita['gambar'])) ?>" alt="Cover" class="img-fluid rounded border shadow-sm" style="max-height: 150px; object-fit: cover; width: 100%;">
                                    <div class="form-text text-success"><i class="ti ti-info-circle me-1"></i>Gambar saat ini aktif.</div>
                                </div>
                            <?php endif; ?>
                            <input type="file" name="gambar" class="form-control" accept=".jpg,.jpeg,.png,.webp">
                            <div class="form-text">Biarkan kosong jika tidak ingin mengubah gambar. Format JPG/PNG/WEBP, maksimal 5MB.</div>
                        </div>
                    </div>
                </div>
            </div>
