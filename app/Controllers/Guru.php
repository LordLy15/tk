...
class Guru extends BaseController
{
    ...
    public function simpan()
    {
        ...
        $existing = $this->guru->where('nip_nik', $nipNik)->first();
        if ($existing) {
            return redirect()->back()
                ->withInput()
                ->with('error', 'NIP/NIK sudah digunakan oleh guru lain.');
        }
    ...
    public function update($id)
    {
        ...
        // Cek NIP/NIK sudah ada atau belum (exclude record saat ini)
        $existing = $this->guru->where('nip_nik', $nipNik)->where('id !=', $id)->first();
        if ($existing) {
            return redirect()->back()
                ->withInput()
                ->with('error', 'NIP/NIK sudah digunakan oleh guru lain.');
